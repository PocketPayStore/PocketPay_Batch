...
public class PointRecoveryStateService {
	...
	public boolean recover(long alertId, long paymentId, long orderId, String completionStep, long staleMinutes) {
		if (!POINT_EARN.equals(completionStep)) {
			throw new IllegalArgumentException("[PointRecovery] 지원하지 않는 후처리 단계입니다: " + completionStep);
		}
		if (mapper.claim(alertId, staleMinutes) == 0) {
			return false;
		}

		String ledgerType = "EARN";
		...
		long amount = Math.round(context.getAmount() * pointEarnRate);
		if (amount <= 0) {
